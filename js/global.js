...
var help_analyse="<p>Vous pouvez analyser votre série en choisissant d’afficher les volées ou les flèches.</p><p>Vous pouvez afficher ou masquer les différentes volées. Des lignes violettes matérialisent la dispersion horizontale et verticale de l’ensemble des volées affichées. Le bouton « Volées » affiche ou masque l’ensemble des volées.</p><p>Si vous ne selectionnez qu’une flèche, vous avez une représentation de son groupement en violet. Si vous appuyez sur le bouton « flèches », vous avez l’ensemble des flèches qui s’affiche ou se masque.</p><p>La zone de réussite est dessinée en vert pointillé, la moyenne de toutes les flèches est dessinée en rose. Ces informations sont également données sous le tableau.</p><p>Le zoom (section à droite) sur la cible est par défaut géré de manière automatique par défaut (font vert), si vous souhaitez le gérer par vous même, utilisez les boutons <span class=\"icon icon-zoom-out\"></span> ou <span class=\"icon icon-zoom-in\"></span>. Ils deviennent alors vert pour indiquer que le zoom est géré manuellement.</p>";
